...
def generateDrops(imagePath, cfg, inputLabel = None):
	...
	for drop in listFinalDrops:
		...
		tmp_output_resized = cv2.resize(tmp_output, (tmp_alpha_map.shape[1], tmp_alpha_map.shape[0]))

		tmp_alpha_map = tmp_alpha_map * (tmp_output_resized / 255)
		...
		if inputLabel is None:
			edge_resized = edge.resize(tmp_alpha_map.size)
			PIL_bg_img.paste(edge_resized, (ix-2*radius, iy-3*radius), tmp_alpha_map)
